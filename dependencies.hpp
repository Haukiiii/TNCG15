...
constexpr float MAX_ABS{ 0.33f }; // Can vary widely depending on the material you are simulating. For common materials, it might range from 0.1 to 10. Materials with higher absorption coefficients will appear darker and more absorbent.
constexpr float MIN_ABS{ 0.1f }; // Should be close to 0, a very small positive number for materials that you want to appear bright and reflective, such as diffuse materials with little absorption.
